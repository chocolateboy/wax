...
Arguments after this are no longer processed by C<wax> and are passed through verbatim e.g.:
    ...
    wax --separator --no-wax command -f http://www.example.com --no-wax --title http://www.example.com
    wax -s -- command -f http://www.example.com -- --title http://www.example.com
...
the system restarts. To save files to another directory, use the C<--directory> option.

=head2 -d, --dir, --directory
...
In addition, each invocation issues a HEAD request to see if each resource has been updated.
...
=item * L<rlwrap|http://utopia.knoware.nl/~hlub/uck/rlwrap/>
